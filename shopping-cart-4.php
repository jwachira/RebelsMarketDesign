<?php include('header.php'); ?>

<div id="content">

  <div class="container headerMarginTop">
  			<div id="top_nav">
		<ul id="purchase_steps">
        	<li id="current"><a class="Shopping_Cart" href="Shopping_Cart.html"><span>1</span>Shopping Cart</a></li>
        	<li><a class="Payment_Method" href="Payment_Method.html"><span>2</span>Payment Method</a></li>
        	<li><a class="Place_Order" href="Place_Order.html"><span>3</span>Place Order</a></li>
        </ul><!-- purchase steps ends -->
    	<div id="product_list_wrapper">
            <div class="lock">
                <h2>Secure Payments</h2>
                <p>This is a secure 128 bit SSL encrypted payment </p>
            </div>  
        </div><!-- product list wrapper ends -->
    </div>
		
		  <section id="cartSection" class="cartSection clearfix">
			  <article class="finalCart clearfix">
				<div class="leftPane">
					<h1>Thank you for your Order</h1>
					<h6>You will recieve a confirmation email shortly email to this address (abc@exmple.com) with order details</h6>
					<p>To ensure you receive our order updates, remember to whitelist our email address (support@rebelsmarket.com),
so that the email is not as spam</p>
					<div class="orderNumber">Order Number: XXXXXXXXXXXX</div>		
				</div>
				
				<div class="rightPane">
					<div class="cartNav">
						<ul>
							<li><a href="#">Track your order</a></li>
							<li><a href="#">Contact Seller</a></li>
							<li><a href="#">Your Wishlist</a></li>
						</ul>
					</div>
				</div>
			 </article>
			<div class="rightWing resetWidthR">
			  <div class="leftCol">
				 <div class="details clearfix">	
					<p>Order from  <span>Lorem Ipsum</span></p>
					<p>Order <span>#H34667572425</span></p>
				 </div>
				 <div class="productPair clearfix">
				   <div class="detaileft">
					 <img src="img/display-tshirt5.jpg" alt="">
					 <button>Remove</button>
				   </div>
				   <div class="detailRight">
					 <h1><strong>Pair High Waisted Cosmic Printed Leggings Branded Waist</strong></h1>
					   <div class="datagrid">
						  <form action="">
							<table>
							<tbody>
							  <tr>
								<td>Size</td>
								<td><strong>S</strong></td>
							  </tr>
							  <tr>
								<td>Quantity</td>
								<td>
									  <select name="">
										<option>1</option>
										  <option>2</option>
										  <option>3</option>
									  </select>
								</td>
							  </tr>
							  <tr>
								<td>Price</td>
								<td>
									<p><strong>$30.00 USD + $0.00 USD Shipping</strong></p>
								</td>
							  </tr>
							</tbody>
						  </table>
						  </form>
					  </div>
				   </div>
				 </div>

			  </div>


			  <div class="rightCol">
					<div class="datagrid">
						<table>
							<tbody>
								   <tr>
									  <td>Order Total</td>
									  <td>$18.00 USD</td>
								  </tr>
								  <tr>
									  <td>Items Total</td>
									  <td>USD 0.00</td>
								  </tr>
								  <tr>
									  <td>Discount</td>
									  <td>$12.00 USD</td>
								  </tr>
								  <tr>
									  <td> <strong>Item Total</strong></td>
									  <td> <strong>$30.00 USD</strong></td>
								  </tr>
							</tbody>
						</table>
					</div>
					<div class="checkMain">
					<p>Let us know how to make your shopping experience better</p>
					<a href="#" class="checkOut resetBtm">Tell us what you think</a>
					</div>
			  </div>
			 
			</div>


			<div class="leftWing resetWidth">
				<h5>Shipping Information</h5>
				<p>
				 1234 Marshall st apt #309 <br />
Redwood City,Ca <br />
Us 94063 <br />
				</p>
			 
			</div>


		  </section> 
		  <section class="otherInterest">
				<h1>You might be interested in these items too</h1>
				<article class="universalSlider">
						<ul class="bxslider" id="universal">
						  <li>
								<div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
						  </li>
						  
						  <li>
								<div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
						  </li>
						  
						  <li>
								<div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
						  </li>
						  
						  <li>
								<div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
								 
								 <div class="productBox galry-item">
									<a href="#" class="pdctImg"><img src="img/display-tshirt.jpg" alt=""></a>
									<div class="displayInfo">
									  <div class="subDetails clearfix">
										<h2><a href="#">Fashion Universe Star</a></h2>
										<span class="dataBy"><a href="#">by MEAN KIDS </a></span>
										<span class="price">USD $ 48.37</span>
									  </div>
									  <div class="shareIcons">
										<a href="#" class="heart">92</a>
										<a href="#" class="star">138</a>
										<a href="javascript:void(0)" class="share">24</a>
									  </div>
									</div>
									<div class="share-popup">
										<a href="#"> <img src="img/facebook-icon.png" alt=""> </a>
										<a href="#"> <img src="img/twitter-icon.png" alt=""> </a>
										<a href="#"> <img src="img/gplus-icon.png" alt=""> </a>
										<a href="#"> <img src="img/pintrest.png" alt=""> </a>
										<a href="#"> <img src="img/email-icon.png" alt=""> </a>
									</div>
									<div class="borderLine"></div>
								 </div>
						  </li>
						  
						</ul>
				</article>
				<div class="btnCenter">
					  <a href="#" class="viewAll">View All</a>
				</div>
		  </section>
 
  </div><!-- end of .container -->

</div><!-- end of #content -->
<?php include('footer.php'); ?>